...
> Técnica → [[vulnerabilities/029-authentication/authentication|entry point]] · ejemplos [[vulnerabilities/029-authentication/examples/001-username-enumeration-brute-force|001]]–[[vulnerabilities/029-authentication/examples/004-password-reset-logica-rota|004]] · labs → [[vulnerabilities/029-authentication/labs/README|labs]] · scripts → [[vulnerabilities/011-brute-force/login_userenum_password.py|brute-force]]
...
**2FA:**
- [ ] **Simple bypass:** completás user+pass y navegás directo a `/my-account`. → [[vulnerabilities/029-authentication/examples/002-2fa-simple-bypass|ejemplo 002]] · [[vulnerabilities/029-authentication/authentication#2️⃣ Fase 2FA / MFA — el segundo factor|técnica]] · [2FA simple bypass](https://portswigger.net/web-security/authentication/multi-factor/lab-2fa-simple-bypass)
- [ ] **Lógica rota:** cookie/param `verify=<víctima>` → brute del código (4 dígitos). → [[vulnerabilities/029-authentication/examples/003-2fa-brute-force-codigo|ejemplo 003]] · [[vulnerabilities/029-authentication/authentication#2️⃣ Fase 2FA / MFA — el segundo factor|técnica]] · [2FA broken logic](https://portswigger.net/web-security/authentication/multi-factor/lab-2fa-broken-logic) · [2FA bypass using a brute-force attack](https://portswigger.net/web-security/authentication/multi-factor/lab-2fa-bypass-using-a-brute-force-attack)
...
- [ ] **Reset poisoning:** en el `POST /forgot-password` inyectá tu host (`TU-collab`) para que el link del mail apunte a vos → el token de la víctima te llega. Probá estos headers (uno por uno, y también combinados): → [[vulnerabilities/016-host-header-injection/examples/002-password-reset-poisoning|host header · 002]]
	...
	- Extra si ninguno pega: `Host: TU-collab`, `Host` duplicado, o `Host: target\n X-Forwarded-Host: TU-collab`.
	→ [[vulnerabilities/029-authentication/authentication#3️⃣ Fase SESIÓN y recuperación — mantener/recuperar la cuenta|técnica]] · Host header → carpeta `vulnerabilities/016-host-header-injection/` · [Password reset poisoning via middleware](https://portswigger.net/web-security/authentication/other-mechanisms/lab-password-reset-poisoning-via-middleware)
...
- [ ] **Fuga del token por parameter pollution (Stage 2):** si un `.js` del reset (ej. `forgotPassword.js`) revela una **API interna** con params `username`/`field`, inyectá codificado en `username`: truncá con `%23` (`#`) y agregá con `%26` (`&`) → `username=administrator%26field=reset_token%23` devuelve el **reset token del admin**. Canjealo en `/forgot-password?reset_token=<robado>`. → [[exam/to-do-list/api-testing#🧬 Server-Side Parameter Pollution (query string)|server-side parameter pollution]] · [lab](https://portswigger.net/web-security/api-testing/server-side-parameter-pollution/lab-exploiting-server-side-parameter-pollution-in-query-string)
