...
# DTDs locales conocidos — wordlist para XXE ciego (DTD local)

> [!info] Qué es y cuándo la usás
> Lista de **rutas de archivos DTD que suelen existir en el disco** de servidores Linux/Java (JBoss, Tomcat, Maven…) y Windows. Sirve para el último recurso del XXE ciego: cuando el server **no sale a internet** y **no hay canal OOB**, reutilizás un **DTD que ya está en el server** y **redefinís una entidad suya** para forzar un leak error-based.
> Técnica completa → [[vulnerabilities/006-xxe/examples/008-xxe-ciego-reutilizar-dtd-local|008 — reutilizar un DTD local]] · entry point → [[vulnerabilities/006-xxe/xxe|XXE]].

> [!tip] Cómo usar la lista
> 1. Por cada ruta, cargala sola para confirmar que **existe** (no da error de "archivo no encontrado"):
>    ```xml
>    <!DOCTYPE foo [ <!ENTITY % local_dtd SYSTEM "file:///RUTA/DEL/DTD"> %local_dtd; ]>
>    ```
> 2. Cuando una **existe**, mirá qué **entidad de parámetro** declara y **redefinila** con el payload error-based del example 008.
> 3. **`docbookx.dtd` (yelp) es el candidato #1** en los labs de PortSwigger — arrancá por ahí; usa la entidad `ISOamso`.

> [!quote] Fuente
> `GoSecure/dtd-finder` → https://raw.githubusercontent.com/GoSecure/dtd-finder/refs/heads/master/list/dtd_files.txt
> Lista verbatim (103 rutas: 97 `.dtd` + rutas de repos Maven `.m2`). Incluye rutas de Windows con `\`.
