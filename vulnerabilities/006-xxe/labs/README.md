...
> Eso es un fingerprint de **[[vulnerabilities/009-server-side-template-injection/server-side-template-injection|SSTI]]** (Jinja2, Twig, Freemarker…), **no** de XXE. XXE no depende de un motor de templates, sino del **parser XML** y de **qué features tiene habilitadas**: resolución de **DTDs**, **entidades externas** (generales / de parámetro `%`), **XInclude**, y —solo en un lab— la presencia de un **DTD local** en el disco del server. Ese es el "detalle de estilo" que distingue cada lab, y va en la columna **Técnica · qué necesitás**.
